...
def get_groups_coco_train_v_sim_t_dissim(model, processor, image_id_2_data, k1, k2, device, batching_config):
    ''' grouping image-text by: first group into visually similar groups, then group into textually similar groups
        ...
        batching_config (dict):
            grouping_function (str): choose from "clip", "blip" 
            ...
            embedding_root (str): path to pre-computed embedding pickles
            ...
    '''
    left_out_ids = [] # storing image_ids from those v_groups that has too little samples to be clustered in the second step
    # compute image and text embeddings
    embedding_root = batching_config['embedding_root']
    if not os.path.exists(os.path.join(embedding_root,'image_embeddings.pickle')):
        if batching_config['grouping_function'] == 'clip':
            image_embeddings, text_embeddings, image_ids = compute_embeddings_clip(image_id_2_data, model, processor, device, batching_config['embedding_mode'])
        elif batching_config['grouping_function'] == 'blip':
            image_embeddings, text_embeddings, image_ids = compute_embeddings_blip(image_id_2_data, model, device, batching_config['embedding_mode'])
        else:
            raise NotImplementedError
        pickle.dump(image_embeddings, open(os.path.join(embedding_root,'image_embeddings.pickle'), "wb"))
        pickle.dump(text_embeddings, open(os.path.join(embedding_root,'text_embeddings.pickle'), "wb"))
        pickle.dump(image_ids, open(os.path.join(embedding_root,'image_ids.pickle'), "wb"))
    else:
        # or load pre-computed image, text embeddings
        print('loading pre-computed embeddings from:', embedding_root)
        image_embeddings = pickle.load(open(os.path.join(embedding_root,'image_embeddings.pickle'),"rb"))
        text_embeddings = pickle.load(open(os.path.join(embedding_root,'text_embeddings.pickle'),"rb"))
        image_ids = pickle.load(open(os.path.join(embedding_root,'image_ids.pickle'),"rb"))

    assert len(image_embeddings) > k1
    image_embeddings = np.array(image_embeddings)

    if 'srl' in batching_config['embedding_mode']:
        # set captions field in image_id2_data with text only, for later data sampling
        for key in tqdm(image_id_2_data.keys()):
            raw_text_captions = []
            for caption_obj in image_id_2_data[key]['captions']:
...
def get_groups_coco_train_t_sim_v_dissim(model, processor, image_id_2_data, k1, k2, device, batching_config):
    ''' grouping image-text by: first group into visually similar groups, then group into textually similar groups
        parameters:
        -----------
        image_id_2_data (dict): {"<image_id>":{"image_path":str,"captions":list(str)}...}
    ...
    if not os.path.exists(os.path.join(embedding_root,'image_embeddings.pickle')):
        ...
        text_embeddings = pickle.load(open(os.path.join(embedding_root,'text_embeddings.pickle'),"rb"))
        image_ids = pickle.load(open(os.path.join(embedding_root,'image_ids.pickle'),"rb"))
    print('image_embeddings length:',len(image_embeddings))
    assert len(image_embeddings) > k1
...
def get_batches_coco_train_random(image_id_2_data, batching_config):
    ...
    for i in range(len(all_pairs)//batch_size + 1):
        next_batch = all_pairs[i*batch_size:(i+1)*batch_size]
...
def batching(image_root, ann_json, batching_config, device, output_dir = None):
    ...
    if batching_config['grouping_function'] == "random":
        inputs = get_batches_coco_train_random(image_id_2_data, batching_config)
        ...
        if batching_config['order'] == 'v_t':
            ...
            image_id_groups, left_out_ids = get_groups_coco_train_t_sim_v_dissim(model, processor, image_id_2_data, k1, k2, device, batching_config)
        else:
        ...
        if len(random_pool) > 0:
            for i in range(len(random_pool)//batch_size + 1):
                next_batch = random_pool[i*batch_size:(i+1)*batch_size]
                if len(next_batch) == batch_size:
                    random_pool_batches.append(next_batch)
        ...
        if mode == 'grouped_only':
            ...
            for i in range(len(flattened)//batch_size + 1):
                next_batch = flattened[i*batch_size:(i+1)*batch_size]
                if next_batch != []:
...
# visualize v_groups and t_groups
