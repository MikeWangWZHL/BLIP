...
def create_dataset(dataset, config, min_scale=0.5):
    ...
    if dataset=='pretrain':
        ...
        print('using training dataset with pre-computed grouping')    
        train_dataset = coco_karpathy_train_with_grouping(
            transform_train, 
            config['image_root'], 
            config['ann_root'], 
            image_id_groups = config['image_id_groups'],
            image_id_2_data = config['image_id_2_data'],
            k1 = config['k1'],
            k2 = config['k2'],
            if_use_all = config['if_use_all']
        )
        val_dataset = coco_karpathy_retrieval_eval(transform_test, config['image_root'], config['ann_root'], 'val') 
        test_dataset = coco_karpathy_retrieval_eval(transform_test, config['image_root'], config['ann_root'], 'test')          
        return train_dataset, val_dataset, test_dataset    
